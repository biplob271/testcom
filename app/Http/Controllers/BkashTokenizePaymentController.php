...
class BkashTokenizePaymentController extends Controller
{
    ...
    public function createPayment(Request $request)
    {
        ...
        $request['intent'] = 'sale';
        $request['mode'] = '0011';
        $request['payerReference'] = $request->order_number;
        $request['currency'] = 'BDT';
        $request['amount'] = $request->amount;
        $request['merchantInvoiceNumber'] =$request->order_number;
        ...
        if ($request->status == 'success'){
            ...
            if (isset($response['statusCode']) && $response['statusCode'] == "0000" && $response['transactionStatus'] == "Completed") {
                 ...
                 * */
                    ...
                    ->where('order_number', $response['payerReference'])
                    ->update(['payment_status' => 'Paid', 'payment_method' => 'Bkash']);
    
         
           return redirect()->route('invoice', $response['payerReference'])->with('success', 'License Successfully Updated');
